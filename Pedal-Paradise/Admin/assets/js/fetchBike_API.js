...
async function responses(ALLBIKEAPIs) {
  const fet = await fetch(ALLBIKEAPIs, {
    method: "GET",
...
async function dataResult(pgNumber) {
  try {
    ...
    obj.sort().forEach((data, index) => {
      ...
      <td class="fw-bold text-primary" style="text-transform: uppercase;">${
        data.bikenumber
      }</td>
    ...
    ${
      data.status
        ? `<input onchange="isStatusUpdate('${data._id}','${data.status}')" style="width:35px;" class="form-check-input" type="checkbox" id="bikeStatus" checked>`
        : `<input onchange="isStatusUpdate('${data._id}','${data.status}')" style="width:35px;" class="form-check-input" type="checkbox" id="bikeStatus">`
    }
    </div>
    </td>
    </tr>`;
...
isStatusUpdate = async (id) => {
  let response = await fetch(
    "http://192.168.29.130:3000/admin/changeBikeStatus/",
    {
      method: "POST",
      headers: {
...
modalImage = () => {
  document.getElementById("imageAppend").innerHTML =
    '<img class="img-fluid" src="./assets/img/KTM_DUKE_200_ABS.png" alt="">';
};
